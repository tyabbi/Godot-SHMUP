...
func _ready():
	...
	iframe_timer.set_one_shot(true)
	iframe_timer.set_wait_time(iframe_delay)
	iframe_timer.connect("timeout", self, "on_iframe_timeout_complete")
...
func on_iframe_timeout_complete():
	iframe = true
